...
class PDFHelper
{
	...
	public static function createSubmissionPDF($submission, $overwrite = false)
	{
        ...
        if (!\JFile::exists(implode($ds, [JPATH_ROOT, $file_path])) || $overwrite)
        {
			...
			self::create($pdf['pdf_filename'], $upload_folder, $pdf_template, $options);
	 ...
	 * @return  void
	 */
	public static function create($filename, $upload_folder, $content, $pdf_options = [])
	{
		...
		$destination_filename = \JPath::clean(implode(DIRECTORY_SEPARATOR, [JPATH_ROOT, $upload_folder, self::makeSafe($filename) . '.pdf']));
